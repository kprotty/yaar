...
impl<T> Receiver<T> {
    pub fn close(&mut self) {
        self.ref_count.fetch_and(!1, Ordering::Relaxed);
    }
    ...
    pub fn poll_recv(&mut self, ctx: &mut Context<'_>) -> Poll<Option<T>> {
        loop {
            ...
            return Poll::Ready(match self.pop(true) {
                Ok(value) => Some(value),
    ...
    fn pop(&mut self, is_ready: bool) -> Result<T, TryRecvError> {
        if let Some(value) = self.local.pop_front() {
            return Ok(value);
        } 

        if is_ready || self.chan.waker.ready() {
            if let Some(value) = self.steal() {
                return Ok(value);
            }

            self.chan.waker.reset();
            if let Some(value) = self.steal() {
                return Ok(value);
            }
        }
